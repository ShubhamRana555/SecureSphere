...
export const registerUser = asyncHandler(async (req, res) => {
  ...
  if (!email || !password || !username || !role || !fullname) {
    return res.status(400).json(new ApiError(400, "Every field is required"));
  }
...
export const resendVerificationEmail = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    return res
      .status(400)
      .json(new ApiError(400, "User not found with this email"));
  }
  ...
  await sendMail({
    email: user.email,
    subject:
      "Resend Email verification for user authentication for SecureSphere App",
    mailGenContent: emailVerificationMailGenContent(
      user.username,
      verificationUrl,
    ),
